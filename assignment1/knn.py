...
for k in k_choices:
    k_to_accuracies[k] = []
    for i in range(num_folds): #loops training over all the folds
        ...
        dists_folds = classifier.compute_distances_no_loops(x_test_fold)
        y_test_pred = classifier.predict_labels(dists_folds,k)
        num_correct = np.sum(y_test_pred == y_test_fold)
        acc = float(num_correct) / y_test_fold.shape[0]
